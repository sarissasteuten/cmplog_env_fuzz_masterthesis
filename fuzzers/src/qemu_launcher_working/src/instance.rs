...
pub struct Instance<'a, EM> {
    options: &'a FuzzerOptions, // comand line options
    mgr: EM, // event manager handles communication between cores
    client_description: ClientDescription, // which core the instance is running on
    #[builder(default)]
...
{
    fn coverage_filter(&self, qemu: Qemu) -> Result<StdAddressFilter, Error> { // not needed to specify anything because default already focusses on target bin only not system or qemu
        ...
        ET: EmulatorModuleTuple<BytesInput, ClientState> + Debug,
    {
        ...
        let map_feedback = MaxMapFeedback::with_name("map_feedback", &edges_observer); // checks if feedback is interesting, it is when a new path is hit in the coverage map, ALSO interesting if it takes unuasually long  (kijk lijn 208)
        let map_objective = MaxMapFeedback::with_name("map_objective", &edges_observer);

        // calibration to get the baseline of coverage
        ...
        let mut feedback = feedback_or!( // checks of de input een new path geeft of juist interesant is door tijd. maar 1 van de 2 hoeft waar te zijn 
        ...
        // A feedback to choose if an input is a solution or not // solution word hier gezien als something notable niet volledige oplossing
        ...
        //IMPORTANT, tokens -> dict that can be used as mutation hints
        ...
        let mut harness = |_emulator: &mut Emulator<_, _, _, _, _, _, _>,
                           _state: &mut _,
                           input: &BytesInput| harness.run(input); // actually running
