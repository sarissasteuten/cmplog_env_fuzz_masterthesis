            ...
            FD_MAPPING.with(|fd_map| {
                ...
                if let Some(real_fd) = fd_map.get(&fd){
                    ...
                    if *real_fd == -1 {
                        ...
                        // println!("print here \n");
                        // qemu.write_reg(Regs::Rdi, &real_fd);
                        let requested_size = _a2 as usize;
                        let mut host_buf = vec![0u8; requested_size];
                        let ret = unsafe {
                            libc::read(*real_fd, host_buf.as_mut_ptr() as *mut libc::c_void, requested_size);
                        };
                        // qemu.write_mem(_a1, &host_buf).unwrap();
            ...
            FD_MAPPING.with(|fd_map| {
                ...
                if let Some(real_fd) = fd_map.get(&fd){
                    ...
                    if *real_fd == -1 {
                        ...
                        let ret = unsafe {
                            libc::fstat(*real_fd, &mut stat_buf);
                        };
                        // qemu.write_function_argument(0, *real_fd as GuestAddr).unwrap();
           ...
           println!("pre hook MMAP\n");
            ...
            FD_MAPPING.with(|fd_map| {
                ...
                if let Some(real_fd) = fd_map.get(&fd){
                    println!("print the real fd {} \n", real_fd);
                    if *real_fd == -1 {
                        return SyscallHookResult::Skip(usize::MAX as GuestAddr);
                    } else {
                        // qemu.write_reg(Regs::Rdi, &real_fd);
                        // qemu.write_function_argument(4, *real_fd as GuestAddr).unwrap();
                        let mut mmap_buf: libc::stat = unsafe { std::mem::zeroed()};
                        let ret = unsafe {
                            libc::mmap(*real_fd, &mut stat_buf);
                        };
                       return SyscallHookResult::Run;
                    }
                }
...
extern "C" fn post_hooks(
    ...
    match sys_num{
        ...
        // n if n == SYS_PREAD64.num =>  {
        //     let real_nbytes = (ret as usize).min(stream::BUCKET_SIZE); // the actual returned size not the requested
        //     let mut fuzzed_bytes = crate::stream::consume_bytes(SYS_PREAD64.num, real_nbytes);
