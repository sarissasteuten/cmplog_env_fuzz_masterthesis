...
fn main(){
    let entries = fs::read_dir("/home/sarissa/Desktop/thesis-sarissa/results/output/client_002/queue/")
        .expect("ohnooooo can not find dir");
    ...
    for entry in entries {
        ...
        let env_vec = bincode::deserialize::<crate::EnvVector>(&bytes)
                        .unwrap_or_else(|_| crate::EnvVector::default_vec());
        
        println!("\n_______env vector {}_______", path.file_name().unwrap().to_str().unwrap());
        println!("sysinfo uptime: {}", env_vec.sysinfo.uptime);
        println!("pid: {}", env_vec.pid_ret);
        println!("ppid: {}", env_vec.ppid_ret);
        println!("access: {}", env_vec.acces_ret);
        println!("getdents: {}", env_vec.getdents_ret);
        println!("nodename: {:?}",  std::str::from_utf8(&env_vec.uname.nodename).unwrap_or("?"));
        println!("sysname: {:?}",  std::str::from_utf8(&env_vec.uname.sysname).unwrap_or("?"));
        println!("stat st_dv: {}", env_vec.stat.st_dev);
        println!("get time of day tvsec: {}", env_vec.gettimeofday_ret.tv_sec);
        println!("get time of day tv usec: {}", env_vec.gettimeofday_ret.tv_usec);
    }
}